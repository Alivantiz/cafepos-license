...
/**
 * То же, но с точностью до минут. «Сегодня» не отличает «три минуты назад» от
 * «утром», а во время аварии решается ровно это: касса ещё звонит или её уже
 * выключили. Для дат старше суток — обычный текст, там минуты не нужны.
 */
export function agoFine(s, empty = 'связи не было') {
  if (!s) return empty
  const m = Math.floor((Date.now() - new Date(s)) / 60000)
  if (m < 0) return 'только что'
  if (m < 1) return 'только что'
  if (m < 60) return `${m} мин назад`
  if (m < 24 * 60) return `${Math.floor(m / 60)} ч назад`
  return agoText(s, empty)
}
